#!/usr/bin/env bash
# Bumps the integration version in manifest.json, verified.
#
...
#
# Usage: bin/bump-version.sh <version>             bump manifest.json to <version>
...
#
# File bumped:
#   - custom_components/fado/manifest.json   (the HA/HACS version of record)
#
...
MANIFEST="custom_components/fado/manifest.json"
require_file "$MANIFEST"

# manifest.json carries the version exactly once. Use sed (not a JSON
# reserialiser) so manifest's required key order is preserved.
