...
async def test_fade_cancel_event_after_brightness_apply(
    ...
    async def cancelling_apply(hass, eid, step, *, use_native_transition=True):
        nonlocal call_count
        call_count += 1
...
async def test_from_parameter_uses_instant_first_step(
    hass: HomeAssistant,
    init_integration: MockConfigEntry,
    service_calls: list[ServiceCall],
) -> None:
    """Test that 'from' parameter causes first step to use transition=0.

    When a from: parameter is specified, the first step should instantly
    move the light to the starting state (transition=0), then subsequent
    steps use the normal transition (0.1s).
    """
    entity_id = "light.test_from_instant"
    hass.states.async_set(
        entity_id,
        STATE_ON,
        {
    ...
    # Call fade with from_brightness_pct - should use instant first step
    await hass.services.async_call(
        DOMAIN,
        SERVICE_FADE_LIGHTS,
        {
            "entity_id": entity_id,
            ATTR_BRIGHTNESS_PCT: 50,
            ATTR_TRANSITION: 1,  # 1 second fade
            "from": {
                ATTR_BRIGHTNESS_PCT: 100,  # Start from 100%
            },
        },
        blocking=False,
    )

    # Wait for at least the first step
    import asyncio

    await asyncio.sleep(0.2)

    # Check service calls - first should have no transition, subsequent should have transition=0.1
    turn_on_calls = [c for c in service_calls if c.service == "turn_on"]
    assert len(turn_on_calls) >= 2, "Should have at least 2 turn_on calls"

    # First call should have no transition parameter (instant)
    assert "transition" not in turn_on_calls[0].data, (
        "First step should not have transition parameter"
    )

    # Second call should have transition=0.1 (normal)
    assert turn_on_calls[1].data.get("transition") == 0.1, (
        "Subsequent steps should use transition=0.1"
    )


async def test_no_from_parameter_uses_normal_first_step(
    hass: HomeAssistant,
    init_integration: MockConfigEntry,
    service_calls: list[ServiceCall],
) -> None:
    """Test that without 'from' parameter, first step uses normal transition.

    When no from: parameter is specified, all steps including the first
    should use the normal transition (0.1s).
    """
    entity_id = "light.test_no_from"
    hass.states.async_set(
        entity_id,
        STATE_ON,
        {
    ...
    # Call fade without from parameter - should use normal transition for all steps
    await hass.services.async_call(
        DOMAIN,
        SERVICE_FADE_LIGHTS,
        {
    ...
    # Check service calls - all should have transition=0.1
    turn_on_calls = [c for c in service_calls if c.service == "turn_on"]
    assert len(turn_on_calls) >= 2, "Should have at least 2 turn_on calls"

    # First call should have transition=0.1 (normal)
    assert turn_on_calls[0].data.get("transition") == 0.1, (
        "First step should use transition=0.1 when no 'from' specified"
    )

    # Second call should also have transition=0.1
    assert turn_on_calls[1].data.get("transition") == 0.1, (
        "Second step should use transition=0.1"
    )
