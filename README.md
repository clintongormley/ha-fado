...
[**Autoconfiguration Panel**](#autoconfiguration-panel) in the Home Assistant
...
See [**Usage: `fado.fade_lights`**](#usage-fadofade_lights) for parameter
specifications.
...
It uses the transition time, [**minimum delay**](#minimum-delay) setting, and
...
time is less than the [**minimum delay**](#minimum-delay), then it sleeps for
...
If the light supports [**native transitions**](#native-transitions) then a short
...
[**`from`**](#starting-values-optional-from-block) parameter is used, the
...
> **Note:** In YAML, bare `on`/`off` parse as booleans, but Fado accepts them
...
When using [native transitions](#native-transitions), the light may emit state
...
> **Note:** Do not use `pip install -e .` (editable install) as it conflicts
